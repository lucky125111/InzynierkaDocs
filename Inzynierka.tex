...
\iffalse
Dodaj tutaj normalny opisik co sie tam dzieje
\fi
...
\subsection{Opis naszego rozwiązania}

Eksplorację danych rozpoczeliśmy od przeglądania obrazów DICOM. To co zauważyliśmy na początku to jakie kłopoty mamy, aby samodzielnie wysegmentować prostate. Trudno jest wyznaczyć granicę pomiędzy narządami w obrazach o tak niskiej rozdzielczości. Przejrzeliśmy również dołączone maski wyznaczone przez lekarzy i stwierdziliśmy, że automatyczne zadanie segmentacji będzie wyzwaniem.
\par
Na początku poszukaliśmy najpopularniejszych algorytmów używanych do segmentacji obrazów. Po zapoznaniu się z kilkoma rozwiązaniami uświadomiliśmy sobie, że segemntacja obazów medycznych jest specyficzna. Zdjęcia są w niskiej jakości, posiadają słaby kontrast oraz problemem jest mała ilość danych treningowych, dlatego zaczeliśmy szukać sieci, która dobrze radzi sobie z dodaktkowo przetworzonymi obrazami. Najodpowiedniejszym rozwiązaniem, które udało nam się znaleźć okzało się zgłoszenie do konkursu Automated Segmentation of Prostate Structures \cite{zgloszenie}. Niestety  na danych testowych rozwiązanie to osiagało współczynnik podobieństwa rowny 0.70 dlatego postanowiliśmy je zmodyfikować według opisu poniżej.
\par
Podczas pierwszej iteracji trenowania okazało się, że czas uczenia sieci zajmuje bardzo dużo czasu na naszych kartach graficznych. Spowodowało to zeskalowanie obrazów do rozdzielczości 96 pixseli na 96 pikseli w celu szybszego prototypowania.

\begin{minipage}{\linewidth}
	\centering
	\includegraphics[width=0.65\textwidth]{segmentation/our_architecture.png}
	\captionof{figure}{Architektura naszej sieci}
\end{minipage}
