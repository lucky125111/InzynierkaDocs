...
\item[U-Net] \hfill \\ 
Sieć U-Net jest zbudowana na podstawie architektury pełnych sieci konwolencyjnych(ang. Fully Convolutional Network) zmodyfikowaną w taki sposób, aby uzyskiwać lepsze rezulataty segmentacji na obrazach medycznych. Lepsze rezultaty pochodzą z kształtu architektury U, który skutkuje większą ilością cech podczas scieżki i lepszym przepływem informacji o kontekście i lokalizacji na obrazie.
...
Na początku poszukaliśmy najpopularniejszych algorytmów używanych do segmentacji obrazów. Po zapoznaniu się z kilkoma rozwiązaniami uświadomiliśmy sobie, że segemntacja obazów medycznych jest specyficzna. Największym problemem jest mała ilość danych treningowych, dlatego zaczeliśmy szukać sieci, która dobrze radzi sobie z danymi sztucznie wygenerowanymi. Najodpowiedniejszym rozwiązaniem, które udało nam się znaleźć okzało się zgłoszenie do konkursu Automated Segmentation of Prostate Structures \cite{zgloszenie}. Niestety nie miało najlepszych wyników na danych testowych dlatego postanowiliśmy je zmodyfikować według opisu poniżej.
...
Do ustalenia skuteczności użwyaliśmy najpopularniejszej miary wykorzystywanej w zadaniach segmentacji tj współczynnik podobieństwa Sørensena. Jest to miara jak podobne są dwa obiekty. Jest to obszar wspólny obszaru segmentacji podzielony przez całkowity rozmiar dwóch obrazów.

Po nauczeniu sieci osiągneliśmy współczynnik podobieństwa Sørensena równy ~0.68 na danych testowych. Jest to wartość lekko niższa niż wartość uzyskana w rozwiązaniu zgłoszonym do konkursu. Różnica może być spowodowana różnymi danymi (zostały one lekko zmienione od czasu konkursu). 

\par

Przykłady poprawnej segmentacji 
...
Przykład problematycznej segmentacji
