...
Objawami świadczącym o problemach z gruczołem prostaty są ból lub pieczenie przy oddawaniu moczu, nieregularne oddawanie moczu i uczucie niepełnego opróżnienia pęcherza. Takie same objawy ma schorzenie nazywane "łagodnym rozrostem gruczołu krokowego". Mężczyźni nie zwykli bagatelizować takich problemów, więc rak prostaty w 75\% jest wykrywany w bezobjawowym stadium ograniczonym do narządu, dzięki czemu łatwo jest go wyleczyć. 
\par
Pierwszym z badań, które jest wykonywane w przypadku podejrzenia występowania nowotworu jest badanie poziomu PSA we krwi. PSA jest antygenem sterczowym, którego podwyższony poziom może świadczyć o rozroście prostaty, zapaleniu lub nowotworze. Jeżeli wykryto podwyższony poziom PSA pacjent kierowany jest na kolejne badania w celu wyjaśnienia przyczyny wystąpienia tej nieprawidłowości.
\par
W wielu przypadkach następnym badaniem jest badanie palpacyjne per rectum. W przypadku tego badania, lekarz szuka nieprawidłowości w kształcie i twardości stercza. Badanie jest obarczone błędem, ponieważ ok. 30\% nowotworów znajduje się w rejonie prostaty nieosiągalnym przez badanie palpacyjne. 
\par
W przypadku podejrzeń nowotworu wykonuje się dwa kolejne badania. Jest to biopsja stercza oraz badanie USG. Biopsja o wyniku pozytywnym jednoznacznie definiuje posiadanie nowotworu. Badanie USG jest mniej dokładne,  jest robione w celu określenia wytycznych do badania MR, czyli rezonansu magnetycznego.
\par
Badanie MR jest dokładne, radiolog za pomocą oprogramowania jest w stanie znaleźć nawet najmniejsze zmiany w strukturze gruczołu. Często po wynikach rezonansu zlecana jest biopsja celowana, która ma sięgnąć do już zbadanych przez rezonans rejonów prostaty. 
...
PSAD, który oblicza się dzieląc stężenia PSA w ng/ml przez objętość stercza. Wynik PSAD pomaga w diagnozie ponieważ udowodniona jest korelacja PSAD z wystepowaniem nowotworu. Korelacja została także stwierdzona z faktem posiadania łagodnego rozrostu prostaty, podczas którego także wydziela się antygen PSA.

\subsubsection{Rezonans magnetyczny}
Rezonans magnetyczny (MRI) to technika obrazowania medycznego stosowanego w radiologii do tworzenia obrazów anatomii i procesów fizjologicznych organizmu. Skanery MRI wykorzystują silne pola magnetyczne, gradienty pola magnetycznego i fale radiowe do generowania obrazów narządów w ciele. MRI nie obejmuje promieni rentgenowskich ani promieniowania jonizującego, co odróżnia je od tomografii komputerowej i skanów PET.
\par
...
W szpitalach używane są trzy metody pomiaru objętości stercza. Jedną z nich jest DRE, (ang. Direct Rectal Exam), a pozostałymi dwoma są metody obrazowania medycznego. Obie opierają się na tych samych obliczeniach, różnią się sposobem pozyskiwania danych. Pierwszym z nich jest badanie MR, a drugim TRUS, czyli przezodbytnicze badanie USG. Kiedy lekarz wykona badanie może przeglądać jego wyniki w odpowiednim programie. Lekarz ogląda gruczoł w różnych płaszczyznach, zaznaczając punkty skrajne. Po zaznaczeniu najbardziej skrajnych punktów wyliczane są trzy wymiary prostaty: szerokość, wysokość i długość. Aby uzyskać objętość prostaty należy zastosować wzór 
...
Na podstawie zebranych danych w ramach przeprowadzonych prac, wśród pacjentów, którzy przeszli zarówno badanie MR jak i badanie TRUS odchylenie standardowe różnicy objętości wynosi 20 $cm^3$. Jest to bardzo dużo, zważywszy na to, że prostata zdrowego mężczyzny powinna mieć objętość poniżej 25 $cm^3$. 

\section{Architektura szpitala}

Architektura szpitalna składa się z sieci zintegrowanych systemów. W poniższej pracy  zlokalizowane zostały komponenty, które będą niezbędne do integracji z naszym systemu. Podstawowym elementem w sieci IT szpitala jest system informacji szpitalnej (HIS), któryI w sumei t koncentruje się głównie na potrzebach administracyjnych szpitali. W wielu wdrożeniach system HIS jest kompleksowym, zintegrowanym systemem informatycznym zaprojektowanym do zarządzania wszystkimi aspektami funkcjonowania szpitala, takimi jak zarządzają danymi demograficznymi pacjentów, ubezpieczeniem informacje, planowaniem leczenia klinicznego, działaniem laboratorium oraz zarządzaniem historią pacjenta i badaniami pacjenta.
...
Nie ma jednoznacznych wytycznych zastosowanych dla dostawców systemów PACS dla opisu zdjęć w formacie DICOM, co utrudnia dane z jednego systemu do pracy w innym systemie. 
\par
...
\section{Metody wspomagania pracy lekarza radiologa}
W dzisiejszym świecie praca radiologa jest ściśle powiązana z pracą przy użyciu dostarczanego oprogramowania zintegrowanego z infrastrukturą szpitalną. Radiolog odczytuje, opisuje i analizuje wyniki badania na komputerze. Jednakże wiele badań dostarczonych jest przez pacjenta w formie papierowej. O problemie znajdowania odpowiednich danych w szpitalnej dokumentacji opowiemy w dalszej części pracy. 
\par
...
Analiza problemu została rozpoczęta przez spotkanie z doświadczonym radiologiem (> 12 lat doświadczenia). Radiolog przedstawił problemy z którymi zmaga się w codziennej pracy i które uważa, że mogłbyby zostać rozwiązane za pomocą dodatkowego oprogramowania. Ze względu na prowadzone prace analityczne w trakcie wytwarzania oprogramowania przyjęta została zwinna metoda wytwarzania oprogramowania. Po finalnym ustaleniu wymagań, jeden z nas udał się do szpitala MSWiA w Warszawie w celu poznania szpitalnej architektury oraz zgromadzeniu danych. 
\par
Praca w szpitalu zaczęła się od gromadzenia danych pacjentów. Następnie poczynione zostały kroki w kierunku integracji docelowego programu wspomagającego pracę radiologa ze szpitalną bazą danych HIS. Ta koncepcja została skazana na porażkę z powodu umowy zawartej między szpitalem a dostawcą oprogramowania. W zawarty jest zapis, iż wszelkie zmiany w kodzie klienta bazy danych mogą być wprowadzane jedynie przez pracowników firmy dostarczającej oprogramowanie.
\par
Kolejnym kierunkiem jaki został przez nas obrany było wyszukiwanie przeglądarki zdjęć DICOM. Jako główne kryterium wyboru zastosowane zostało kryterium języka programowania, jakim jest Python, oraz łatwa rozszeżalność rozwiązania. Po znalezieniu odpowiedniej przeglądarki nastąpiła próba instalacji tej przeglądarki na szpitlanych komputerach. Próba zakończyła się porażką z powodu prawnych regulacji dotyczących bezpieczeństwa danych pacjentów znajdujących się na szpitalnych serwerach.
\par
Następnym rozwiązaniem, które zostało przez nas opracowane było kopiowanie zanonimizowanych danych pacjentów na dysk twardy i przenoszenie ich na komputer, który nie jest podłączony do sieci szpitalnej. 
...
\par
Nie została wyrazona zgoda na instalację zewnętrznego oprogramowania na komputerach podłączonych do sieci szpitalnej. Jest to ruch wynikający z regulacji prawnych dotyczącyh ochrony wrażliwych danych pacjentów. Otwarte oprogramowanie używane przez nas mogłoby naruszyć regulacje bezpieczeństwa szpitala. Po podjęciu decyzji o przenoszeniu danych nośnikiem fizycznym należało się skupić na module anonimizującym. Szpitalna baza danych ma taki moduł, więc dane pacjentów kopiowane na dysk twardy są już anonimowe. Został użyty dysk twardy, ponieważ kompleksowy plik zawierający badanie jednego pacjenta zajmuje średnio ponad Gigabajt danych. 
\par
...
\par
Uzyskane rozwiązanie jest dalekie od optymalnego. Nie jest ono ani wygodne ani szybkie. Jest natomiast bezpieczne. Przy pracy z bardzo delikatnymi danymi kryterium bezpieczeństwa musi być priorytetem. 
\par
...
\par
Największym kłopotem związanym z wykonywaniem tego rodzaju pracy był fakt, iż każdy szpital używa innego formatowania dokumentów, więc nierzadko występowały trudności z odczytaniem istotnych w naszym badaniu informacji. 
\par
Po uzupełnieniu arkusza kalkulacyjnego należało jeszcze wykonać niezbędne obliczenia objętości oraz PSAD. 
\par
Jedną z dróg, które były rozwarzane w początowych fazach projektu było użycie systemu OCR na zeskanowanych kartach pacjentów. System OCR (z ang. Optical Character Recognition) to oprogramowanie pozwalające na znajdowanie i rozpoznawanie fragmentów tekstu w plikach graficznych. Należało porzucić tę drogę z powodu niejasności kart pacjentów i niejednoznacznych wyników oraz korekt niejednokrotnie nanoszonych długopisem przez lekarza. 
\par
Taka procedura pozyskiwania danych jest wyjątkowo nieoptymalna oraz podatna na błędy. Przy obecnej infrastrukturze szpitalnej ręczne przepisywanie danych jest jedynym sposobem na digitalizację zasobów pacjentów. Komfort pracy lekarzy znacząco by się podniósł gdyby zostały wyeliminowane wszystkie papierowe karty pacjentów. Wypełnienie danych jednego pacjenta w tym badaniu było procesem trwającym 15 minut. Lekarz radiolog dziennie wykonuje około 4 badań MR (w szpitalu MSWiA), więc można założyć, że marnuje około godziny dziennie na niepotrzebne przeglądanie papierowej dokumentacji. Po konsultacjach z doktorem podjęta została decyzja aby stworzyć program, który pozwoli na oszczędzanie tego czasu. Wszyscy lekarze którzy dostarczyli swojej ekspertyzy przyznali, że w codziennych obowiązkach pomógłby im program wyliczający objętość prostaty na podstawie plików DICOM. Jest to jedna z płaszczyzn, na których informatyka jest w stanie pomóc oszczędzać czas lekarzy, co przekłada się na komfort oraz zdrowie pacjentów. Problem za bardzo poważny, dlatego właśnie obrana została realizacja tego tematu. 
\par
...
\end{description}
\par
Tablica 2.1 przedstawia wybrane kolumny uzyskane z danych szpitalnych
\\
\begin{table}[h!]
...
Tabela z podstawowymi statystykami zebranymi z utworzonego zbioru danych
\begin{table}[h!]
	\caption{Podstawowe statystyki}
	\centering
	\begin{tabular}{|p{5cm}|l|} \hline  
		Liczba pacjentów & 222  \\ \hline 
		Średni wiek pacjenta & 67 lat  \\ \hline 
...
\begin{minipage}{\linewidth}
	...
	\captionof{figure}{PSA względem Gleasona}
\end{minipage}
\\
...
Głęboka sieć neuronowa (DNN) to sztuczna sieć neuronowa z wieloma warstwami między warstwami wejściową i wyjściową. DNN znajduje poprawną matematyczną manipulację, aby przekształcić dane wejściowe na dane wyjściowe, niezależnie od tego, czy jest to relacja liniowa czy nieliniowa. Sieć przechodzi przez warstwy obliczające prawdopodobieństwo każdego wyjścia. Każda manipulacja matematyczna jako taka jest uważana za warstwę, a złożone DNN mają wiele warstw, stąd nazwa "głębokie" sieci. Celem jest, aby ostatecznie sieć została nauczona w zakresie rozkładania obrazu na funkcje, identyfikowania trendów istniejących we wszystkich próbkach i klasyfikowania nowych obrazów według ich podobieństw bez konieczności wprowadzania danych przez człowieka.
\par
Konwolucyjna  sieć neuronowa (CNN) to rodzaj sztucznej sieci neuronowej wykorzystywanej w rozpoznawaniu i przetwarzaniu obrazów, która jest specjalnie zaprojektowana do przetwarzania obrazów. W przeciwieństwie do sieci użytej do głębokiego uczenia maszynowego, każda ukryta warstwa aktywacyjna
jest obliczany jako iloczyn kartezjański wyników kolejnych wartstw. Jednak w sieciach konwolucyjnych każda ukryta aktywacja jest obliczana przez pomnożenie małego lokalnego sygnału wejściowego względem ciężarów. Ciężary są następnie dzielone na całej przestrzeni wejściowej. Po obliczeniu ukrytych jednostek, maksimum
warstwa pomaga usunąć zmienność ukrytych jednostek. W szczególności każda jednostka gromadząca maksimum otrzymuje aktywacje ze znalezionej ilości splotowych pasm, i wyprowadza maksimum aktywacje z tych splotów. Większość CNN działa w rozpoznawaniu obrazu ma niższe warstwy sieciowe, które są splotowe, a wyższe warstwy sieciowe są w pełni połączone. W tej sekcji omówimy ile potrzeba warstw splotowo-w pełni połączonych, co to optymalna liczba ukrytych jednostek na warstwę, co jest najlepsze strategia łączenia i najlepszy typ wejścia dla CNN. tradycyjnych sieciach neuronowych. \\

\begin{description}
\iffalse
\item[Pełna sieć konwolucyjna (ang. FCN )]  \hfill \\
	Nazwa pełnych sieci konwolucyjnych pochodzi od warstw wykorzystanych w modelu. Sieć ta składa się w pełni z warstw konwolucyjnych co oznacza, że składa się z samych filtrów. Ponieważ nie posiada żadnych gęstych warstw, przyjmuje obrazy wejściowe w każdej rozdzielczości.
...
\end{description}
	...
	Strzałki na rysunku oznaczają połączenia pomiędzy warstwami odpowiedzialnymi za kontekst a warstwami odpowiedzialnymi za informacje przestrzenne. Na tej podstawie sieć decyduje czy konkretny lokalny region jest obszarem  do segmentacji.
...
W przypadku analizy obrazów medycznych problemem jest mały zbiór danych testowych dobrej jakości. Postanowiliśmy do tego celu wykożystać dane pochodzące z konkursu opisane powyżej. Eksplorację danych rozpoczeliśmy od przeglądania obrazów DICOM. Na wstępie zauważyliśmy, że granice pomiędzy tkankami miękkimi nie są dobrze podkreślone. Co więcej dane jakie dostaliśmy są niskiej rozdzielczości. Segmentacja gruczołu krokowego nie jest zadaniem trywialnym z powodu podobnych intensywności obrazu między gruczołem prostaty a sąsiadującą tkanką oraz z powodu niejednorodności intensywności obrazu w okolicy prostaty. Co więcej, gruczoł krokowy ma dużą różnorodność wyglądu i kształtów u różnych pacjentów.
Z uwagi na bardzo ograniczony zbiór testowy zastosowaliśmy mechanizmy \textit{Data augmentation}, dzięki czemu w efektywny spoób zwiększyliśmy zbiór testowy. Rowiązanie jest standarowym procesem stosowanym przy przetwarzaniu obrazów medycznych, polega na zwielokrotnienu zbioru testowego poprzez zastosowanie na obrazach inicjalnych podstawowych operacji przetwarzających obraz takich jak np. rotowanie obrazu, sotsowanie filtru Gaussowskiego lub progowania.
\par
...
Kolejne dwa bloki zawierają identyczne warstwy. Te 3 bloki są nazywane ścieżką zstępującą. Jej zadanie to ustalenie znaczenia danych na obrazie. Następnie ten kontekst będzie przetransformowany do ścieżki wschodzącej.

Następnie znajduje się bottleneck 
...
Tę część sieci nazywamy ścieżką wstępująca odpowiedzalną za ustalenie dokładnej lokalizacji segmentacji.
...
Po augmentacji obrazów zaczeliśmy dobieranie parametrów sieci. Rozpoczeliśmy sprawdziań krzyżowy parametrów sieci. Na tej podstawie ustaliliśmy, że odpowiednia liczba epok wynosi 30 natomiast batch\_size=32.
...
Do ustalenia skuteczności użyliśmy najpopularniejszej miary wykorzystywanej w zadaniach segmentacji tj. współczynnika podobieństwa Sørensena. Miara ta mówi jak podobne są dwa obiekty. Jej wartość to wspólny obszar regionu segmentacji podzielony przez całkowity rozmiar dwóch obrazów.
